...
class travel(pt.behaviour.Behaviour):
	...
	def __init__(self, name, max_rotate_count, max_translate_count, destination):
		...
		self.stop_msg = Twist()
		self.first_try = False # Is used for the "go to pick" node, so that we get success when we are already there.
		self.reset = False
		self.reset_pose = False
		
		# If our destination is the start we set the "self.first_try" to True.
	...
	def update(self):
		...
		# If self.reset is True, meaning the cube was not found in placement area.
		if self.reset:
			self.rotate_count = 0
			self.translate_count = 0
			self.at_destination = False
			...
			# If we need to go back to start, also reset pose.
			if self.destination == "pick":
				goal = PlayMotionGoal()
		...
		# If we tick the go_to_pick node first time and we are already there.
		if self.first_try:
		...
		# Send rotate messages until we have rotated "enough".
...
class C_level_BehaviourTree(ptr.trees.BehaviourTree):
	...
	def __init__(self):
		...
		# Define some global variables that are used "frequently" so we do not need to pass them.
		...
		global_cube_reset_msg.pose.orientation.y = 0
		global_cube_reset_msg.pose.orientation.z = 0
		global_cube_reset_msg.pose.orientation.w = 1
		...
		# Go to pick. Will only be performed if the cube is not detected in final area.
		branch_1 = travel("go to pick", 30, 9, "pick")
		...
		# Pick up Cube
		branch_3 = pick_up('pick_up', self.pick_srv_nm, self.aruco_pose_pub)
		...
		# Perform a final check for the cube.
		branch_6 = final_check("final check", self.aruco_pose_top)

		# become the tree
		tree = RSequence(name="Main sequence", children=[branch_0, branch_1, branch_2, branch_3, branch_4, branch_5, branch_6])
