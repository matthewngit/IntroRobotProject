...
class final_check(pt.behaviour.Behaviour):
	...
	def update(self):
		...
		if not global_cube_pose:
			...
			# try to move slightly back so we don't crash into table with arm
			"""
			move_msg = Twist()
...
class place(pt.behaviour.Behaviour):
	...
	def update(self):
		...
		if self.place_request.success:
			...
			# try to spin to prevent crashes with arm
			"""
			move_msg = Twist()
			move_msg.angular.z = 1
